...
def bits_needed(x):
    # menghitung kemungkinan jumlah bit yang dibutuhkan berdasarkan jumlah imputan
    # misal => inputan group = 5 inputan berarti binary yang dibutuhkan sebanyak 3 deret (000) 1+2+4 = 8 
...
def convert_input_to_bin():
    ...
    print("nilai cpg (perhatikan index) :", cpg) # liat index
    # print("cpg[_c])[2:] =",cpg[_c])[2:])
    # for _c in range(len(cpg)):
    #     print("cpg[_c])[2:] :",bin(cpg[_c])[2:])       
        
    for _c in range(len(cpg)): # mengulang sebanyak panjang cpg;
        print("nilai _c :",_c) 
        if _c % 3:  # CourseClass 
            print("nilai modulus ",_c," %3:",_c%3)
            print("cpg[",_c,"]->sebelum bits_needed =",cpg[_c])
            print("bin(cpg[",_c,"]) = ",bin(cpg[_c]))
            print("bin(cpg[",_c,"])[2:] = ",bin(cpg[_c])[2:])
            cpg[_c] = (bin(cpg[_c])[2:]).rjust(bits_needed(CourseClass.classes), '0')
            print("nilai cpg index",_c ," % 3 --> Courseclass=",cpg[_c])
            # sys.exit()
        elif _c % 3 == 1:  # Professor
            cpg[_c] = (bin(cpg[_c])[2:]).rjust(bits_needed(Professor.professors), '0')
            print("nilai cpg index",_c," %3 = 1 --> professor :",cpg[_c])
        else:  # Group
            print("cpg[_c]->sebelum bits_needed =",cpg[_c])
            print("bin(cpg[_c]) = ",bin(cpg[_c]))
            print("bin(cpg[_c])[2:] = ",bin(cpg[_c])[2:])
            cpg[_c] = (bin(cpg[_c])[2:]).rjust(bits_needed(Group.groups), '0')
            print("nilai cpg index",_c," else=",cpg[_c])
    print("full index cpg:", cpg)
